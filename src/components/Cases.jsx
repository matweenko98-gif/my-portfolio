      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {cases.map((project, idx) => {
          ...
          return (
            <article
              key={idx}
              className="bg-gray-50/40 border border-neutral-200/80 rounded-lg p-3 flex flex-col justify-between group transition-all duration-300 hover:bg-white hover:shadow-lg hover:shadow-gray-100/40 cursor-pointer"
            >
              <div>
                {/* Внутренний таб-индекс */}
                <span className="text-[10px] font-medium tracking-wider text-gray-400 uppercase mb-3 block">
                  [ Кейс {caseNumber} ]
                </span>

                {/* Графический контейнер с эффектом увеличения при ховере */}
                <div className="relative overflow-hidden aspect-[4/3] rounded-lg bg-zinc-50 border border-zinc-100/50 transition-all duration-500 group-hover:scale-[1.01]">
                  ...
                  <div className="absolute bottom-3 left-3 flex flex-row flex-wrap gap-1.5 z-10">
                    {project.tags.map((tag, tIdx) => (
                      <span
                        key={tIdx}
                        className="bg-white/10 backdrop-blur-md border border-white/20 text-white text-[10px] px-2.5 py-1 rounded-sm font-medium shadow-sm tracking-wide uppercase"
                      >
                        {tag}
                      </span>
                    ))}
                  ...
                  <div className="hidden md:flex absolute inset-0 items-center justify-center transition-all duration-500 opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 z-20 bg-black/10 backdrop-blur-sm">
                    <span className="text-white font-medium text-sm border border-white/30 bg-white/10 px-4 py-2 rounded-sm backdrop-blur-md shadow-lg tracking-tight">
                      Смотреть кейс →
                    </span>
                  </div>
